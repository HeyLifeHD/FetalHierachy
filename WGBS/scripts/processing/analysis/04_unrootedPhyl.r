...
for(j in names(col)){
    ...
    colors <- col[[j]]
    pheno$colors_oi <- colors[match(as.character(pheno[,j]), names(colors))]
    colors_final <- pheno[, "colors_oi", drop=FALSE]
    ...
    for(i in names(dmrs_final)){
        ...
        pdf(file.path(analysis.dir, i, "visualization",paste0("Phyl_DMR_man_ward2_",j,".pdf")),width=10, height=10)
        ...
        dev.off()
        pdf(file.path(analysis.dir, i, "visualization",paste0("Phyl_DMR_man_ward2_",j,"_label.pdf")),width=10, height=10)
